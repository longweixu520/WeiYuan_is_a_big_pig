...
public:
    ...
    */
    ...
    list<memNode*>::iterator it = _empty.begin();
    memNode* bestFit = nullptr;
    int minWaste = INT_MAX;

    // 遍历空闲内存块，选择最小符合要求的内存块
    for (; it != _empty.end(); ++it) {
        int availableSize = (*it)->get_size();
        if (availableSize >= size) {  // 符合内存请求的大小
            int waste = availableSize - size;  // 计算浪费的空间
            if (waste < minWaste) {  // 选择浪费最小的块
                minWaste = waste;
                bestFit = *it;
            }
        }
    ...
    if (bestFit != nullptr) {
        // 如果找到了合适的内存块进行分配
        memNode* allocatedBlock = new memNode;
        allocatedBlock->set_node(bestFit->get_begin(), bestFit->get_begin() + size);
        bestFit->cut_node(size);  // 剩余内存切割

        _use.push_back(allocatedBlock);  // 将分配的内存块加入已用内存链表

        cout << "分配成功！" << endl;
    } else {
        cout << "没有足够的空闲内存块可以分配！" << endl;
    }
}
...
*/
    ...
    while(1){
        ...
        switch(c){
        case 1:
            distribution(NO_LOOP);  // 使用最佳适应算法进行分配
            break;
        case 2:
            freed();  // 释放内存
            break;
